...
2. **Build a structured profile** with the persona creator: the `profiling_prompt` template in the model config (e.g. `configs/model_configs/gpt5_config.yaml`) turns a free-text description into a profile, applied by the `Profiler` class (`src/vibe_testing/profiling.py`) via `scripts/stage_1_profile_user.py` — producing input preferences (what to test) and output preferences (how to judge).
